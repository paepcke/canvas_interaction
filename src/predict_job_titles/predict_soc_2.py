...
glove_dict = glove2dict('/dfs/scratch0/aprilyu/glove.6B/glove.6B.%dd.txt' % input_size)
with open('/tmp/aprilyu.log', 'w') as out:
	out.write("read in glove vectors\n")
	out.flush()
	...
	for s in soc.find().batch_size(500):
		...
		for w in s['filtered_title'].split():
			...
			if w in glove_dict.keys():
				...
				out.write("couldn't find word: %s\n" % repr(w))
				out.flush()
	...
	out.write(loss_and_metrics)
	out.flush()

with open('/dfs/scratch0/aprilyu/trained_nn.pickle', 'wb') as nn_out:
	pickle.dump(model, nn_out)
